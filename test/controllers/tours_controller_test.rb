...
class ToursControllerTest < ActionController::TestCase
  ...
  test "should no create a order in a marketplace if not found on moip" do
    @payment_data["id"] = @mkt
    post :confirm_presence, @payment_data
    assert_equal assigns(:confirm_headline_message), "Não foi possível confirmar sua reserva, conta do organizador não encontrada"
    assert_equal assigns(:confirm_status_message), "A conta do organizador não foi encontrada"
    assert_equal assigns(:status), "success"
    assert_equal flash[:error], "A conta Moip informada não foi encontrada"
