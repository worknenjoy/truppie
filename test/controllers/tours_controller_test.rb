...
class ToursControllerTest < ActionController::TestCase
  ...
  setup do
    ...
    @basic_tour_fair_price = {
      title: "A basic truppie with fair price",
      organizer_id: Organizer.last.id,
      where_ids: [Where.last.id],
      user_id: User.first.id,
      start: Time.now,
      end: Time.now,
      value_chosen_by_user: true
    }
  ...
  test "should create without price but with flag for allowing user to set 'fair price'" do
    post :create, tour: @basic_tour_fair_price
    assert_equal 'Truppie criada com sucesso', flash[:notice]
  end
  ...
  test "test a start confirmation of a external product from rezdy" do
    post :confirm_product, {id: "PWQQVJ", name: "foo", prices: ["1", "2", "3"], labels: ["children", "adult", "adult"], starts: ["343434", "123", "322"], products: ["168282", "1809756", "168282"] }
    assert_equal assigns(:product_id), "PWQQVJ"
    assert_equal assigns(:product_name), "foo"
    assert_equal assigns(:product_prices), ["1", "2", "3"]
    assert_equal assigns(:product_labels), ["children", "adult", "adult"]
