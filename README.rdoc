{<img src="https://travis-ci.org/alexanmtz/truppie.svg?branch=master" alt="Build Status" />}[https://travis-ci.org/alexanmtz/truppie]

https://truppie.com/assets/logo_truppie-839885d09468eb209481b15d636391314cfa8e6cb9c1cb61c24f1833c0dca023.svg

The trupe hippie chic exploring adventures platform: https://www.truppie.com

= Be a Truppist!

  Welcome to the Truppie platform
  
  Here you will know how contributing to Truppie.
  
  This project is Open Source and works with developers around the world.
  Any developer can contribute with this project and be assigned to to the issues.
  
== What it is?
  
  Truppie is a project that connects tour guides with explorers.
  The development of the platform is made by developers like you.
  
  Truppie is safe, we use ssl for all transactions.
  
== How it works
  
  Choose the Issues, be assigned and send a pull request.
  
  We will analyze and if is good to merge, it will goes to production and you will be paid por it.
  
  You can request a Bug fix, creating a new issue and be paid for it.
  
  So if you are not a developer, actually you can be paid if we accept that feature too.
  
== Advantages

  As developer, you can learn with a collaborative platform!
  
  Be paid on demmand for new features, bug fixes and improvements of the project
  
== Requirements
  
  To work with us, you need to be a developer that works well with TDD and BDD.
  
== Installing

=== Env variables to secure our working
  
  When you have the project installed, you can request the development env variables

=== RELEASE NOTES

- 2.0.rc1 
  - #1: Use Stripe as the main payment platform 
  - Continuous Integration system with Heroku pipeline, Github and Travis CI
  - SSL support   

